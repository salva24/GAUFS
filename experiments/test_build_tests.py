from experiments.build_test_genetic_2 import *
# from analysis_weighted_variables_num_cluster import analyse_dataset
from analysis_of_genetic_search import analyse_dataset
import json
...
def main():
    ...
    for file in tqdm(os.listdir(path_datasets)):
        ...
        test = BuildTest(name_file,parallel_evaluation=True)
        ...
        for fitness in fitness_internos:
            for linkage in linkages:
                analyse_dataset(name_file,max_num_considerado_clusters=test.rango_banda_clusters[0][1],fitness=fitness,linkage=linkage, parallel_evaluation=True)
